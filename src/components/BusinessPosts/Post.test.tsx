...
describe("Post Component", () => {
  ...
  test("handles pinning a post", () => {
    // Render the Post component with the necessary props
    render(
      <Post
        post={mockPost}
        media={mockMedia}
    ...
    // Simulate a click on the "Pin Post" button
    fireEvent.click(screen.getByTitle("Pin Post"));

    // Assert that the onPin function was called with the correct postId and "true" for pinned status
    expect(mockOnPin).toHaveBeenCalledWith(mockPost.postId, "true");
  });

  test("handles unpinning a post", () => {
    // Create a mock post that is initially pinned
    const pinnedPost = { ...mockPost, isPinned: true };

    // Render the Post component with the pinned post and necessary props
    render(
      <Post
        post={pinnedPost}
        media={mockMedia}
        ...
        onPin={mockOnPin}
        isEditing={false}
        editTitle={pinnedPost.title}
        editContent={pinnedPost.content}
        setEditTitle={mockSetEditTitle}
        setEditContent={mockSetEditContent}
        handleUpdate={mockHandleUpdate}
      />
    ...
    // Simulate a click on the "Unpin Post" button
    fireEvent.click(screen.getByTitle("Unpin Post"));

    // Assert that the onPin function was called with the correct postId and "false" for pinned status
    expect(mockOnPin).toHaveBeenCalledWith(mockPost.postId, "false");
  });
});
